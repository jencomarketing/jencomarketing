      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-md animate-fade-in">
        <div className="bg-cyberGray p-8 rounded-2xl border border-brandOrange/30 shadow-2xl w-full max-w-sm relative">
          ...
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-black text-white font-tech">Admin Login</h2>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <input 
                ...
                className="w-full bg-black border border-white/20 rounded-lg p-3 text-white focus:border-brandOrange outline-none font-mono"
              />
            </div>
            <div>
              <input 
            ...
            <button type="submit" className="w-full bg-brandOrange text-white font-bold py-3 rounded-lg hover:bg-brandOrange/90 transition-colors shadow-glow-orange font-tech">
              登入 Login
            </button>
          </form>
        </div>
        ...
        <div className="bg-cyberGray p-6 rounded-xl border border-brandOrange w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl animate-fade-in">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xl font-bold text-white flex items-center gap-2 font-tech">
              <Download className="text-brandOrange" /> Export Data
            </h3>
          ...
          <button 
            onClick={copyExportCode}
            className={`w-full py-4 rounded-lg font-bold text-lg flex items-center justify-center gap-2 shadow-lg transition-all font-tech ${copied ? 'bg-green-600 text-white' : 'bg-brandOrange text-white hover:bg-brandOrange/90'}`}
          >
            {copied ? (
              <>
    ...
    <div className="fixed inset-0 z-[60] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-cyberGray border-l border-brandOrange/30 h-full overflow-y-auto shadow-2xl animate-slide-in flex flex-col">
        <div className="sticky top-0 bg-cyberGray/95 backdrop-blur z-10 border-b border-white/10 p-4 flex justify-between items-center flex-shrink-0">
          <h2 className="text-xl font-bold text-white flex items-center gap-2 font-tech">
        ...
        <div className="p-4 flex-1 overflow-y-auto">
          ...
          <div className="flex gap-2 mb-6 overflow-x-auto pb-2 scrollbar-hide">
             ...
             <TabButton active={activeTab === 'portfolio'} onClick={() => setActiveTab('portfolio')} label="作品" icon={<Video size={14} />} />
          </div>
            ...
            <div className="space-y-4">
              <InputGroup label="网站 Title (Browser Title)">
                <input 
                  ...
                  className="w-full bg-black border border-white/20 rounded p-2 text-white focus:border-brandOrange outline-none font-mono text-sm"
                />
              </InputGroup>
            ...
            <div className="space-y-8">
              {content.cases.map((c, idx) => (
                <div key={c.id} className="border border-white/10 rounded-lg p-4 bg-black/20">
                  <h3 className="text-brandYellow font-bold mb-3 font-mono">Case #{idx + 1}</h3>
                  <div className="space-y-3">
                    <div>
                      <label className="text-xs text-gray-400 font-mono">商家 Name</label>
                      <input 
                    ...
                    <div>
                      <label className="text-xs text-gray-400 font-mono">Result (e.g. Sales 翻倍)</label>
                      <input 
                        className="w-full bg-black border border-white/10 rounded p-2 text-sm text-white"
                        value={c.result}
                    ...
                    <div>
                      <label className="text-xs text-gray-400 font-mono">Story 描述</label>
                      <textarea 
                        className="w-full bg-black border border-white/10 rounded p-2 text-sm text-white"
                        rows={3}
                        value={c.desc}
                    ...
                    <div>
                      <label className="text-xs text-gray-400 block mb-2 font-mono">Change Photo</label>
          ...
          {activeTab === 'partners' && (
            <div className="space-y-6">
              <div className="bg-black/40 p-4 rounded border border-white/10">
                <h3 className="text-sm font-bold text-white mb-3 font-mono">Add New Partner (新增商家)</h3>
                <div className="space-y-3">
                   <input 
                    type="text" 
                   ...
                   <button 
                    ...
                    className="w-full bg-brandOrange text-white font-bold px-3 py-2 rounded text-sm hover:bg-brandOrange/80 font-tech"
                   >
                     Add Partner
                   </button>
                </div>
              ...
              <h3 className="text-sm font-bold text-gray-400 mt-6 mb-2 font-mono">Edit Partners List (现有商家)</h3>
          ...
          {activeTab === 'portfolio' && (
            <div className="space-y-6">
              <div className="bg-black/40 p-4 rounded border border-white/10">
                <h3 className="text-sm font-bold text-white mb-3 font-mono">Add New Work (上传作品)</h3>
                <div className="space-y-4">
                  <div>
                  ...
                  <button 
                    ...
                    className="w-full bg-brandOrange text-white font-bold px-3 py-3 rounded text-sm hover:bg-brandOrange/80 shadow-glow-orange flex justify-center gap-2 font-tech"
                  >
              ...
              <div className="space-y-2">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider font-mono">Current Portfolio (现有作品)</h3>
        ...
        <div className="p-4 border-t border-white/10 bg-cyberGray/95 backdrop-blur z-10 flex flex-col gap-3">
          <button 
            ...
            className="w-full bg-green-600 text-white font-bold py-3 rounded hover:bg-green-500 transition-colors flex items-center justify-center gap-2 shadow-lg animate-pulse-glow font-tech"
          >
          ...
          <button 
            ...
            className="w-full flex justify-center items-center gap-2 text-red-500 hover:text-red-400 text-xs py-2 font-bold bg-red-500/5 hover:bg-red-500/10 rounded font-mono"
          >
...
const TabButton = ({ active, onClick, label, icon }: any) => (
  <button 
    onClick={onClick}
    className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition-all whitespace-nowrap font-tech ${active ? 'bg-brandOrange text-white shadow-glow-orange' : 'bg-black text-gray-400 border border-white/10 hover:border-white/30'}`}
  >
    {icon} {label}
  </button>
);
...
const InputGroup = ({ label, children }: { label: string, children: React.ReactNode }) => (
  <div>
    <label className="block text-sm font-bold text-gray-300 mb-2 font-mono">{label}</label>
    {children}
  </div>
);